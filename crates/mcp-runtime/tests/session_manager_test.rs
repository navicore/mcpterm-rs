...
async fn test_session_manager_text_response() {
    ...
    model_tx
        .send(ModelEvent::LlmMessage(response.content.clone()))
        .unwrap();
...
async fn test_session_manager_streaming_response() {
    ...
    let model_tx = session_manager.get_model_sender();
    model_tx
        .send(ModelEvent::LlmStreamChunk("Hello, ".to_string()))
        .unwrap();
    model_tx
        .send(ModelEvent::LlmStreamChunk("I'm an AI ".to_string()))
        .unwrap();
    model_tx
        .send(ModelEvent::LlmStreamChunk("assistant!".to_string()))
        .unwrap();
    model_tx.send(ModelEvent::LlmResponseComplete).unwrap();
...
async fn test_session_manager_tool_calls() {
    ...
    if !response.tool_calls.is_empty() {
        ...
        model_tx
            .send(ModelEvent::ToolResult(tool_call.tool.clone(), tool_result))
            .unwrap();
        ...
        model_tx
            .send(ModelEvent::LlmMessage(response2.content.clone()))
            .unwrap();
        model_tx.send(ModelEvent::LlmResponseComplete).unwrap();
    }
