...
async fn test_three_client_event_bus() -> Result<()> {
    ...
    let ui_component = {
        ...
        move |input: &str| -> Result<()> {
            ...
            event_bus.ui_sender().send(UiEvent::UserInput(input.to_string()))?;
            Ok(())
        }
    };
    ...
    let _model_component = {
            ...
            create_handler(move |event: UiEvent| {
                ...
                Box::pin(async move {
                    if let UiEvent::UserInput(input) = &event {
                        ...
                        event_bus.model_sender().send(ModelEvent::LlmMessage(response))?;
                    }
                    Ok(())
                })
            })
        ...
        move |response: &str| -> Result<()> {
            ...
            event_bus.model_sender().send(ModelEvent::LlmMessage(response.to_string()))?;
            Ok(())
        }
    };
    ...
    let _api_component = {
        ...
        move |message: &str| -> Result<()> {
            ...
            event_bus.api_sender().send(ApiEvent::SendRequest(message.to_string()))?;
            Ok(())
        }
    };
    ...
    match result {
        Ok(_) => {
            ...
            assert!(ui_events.lock().unwrap().len() >= 2,
                   "UI should have registered at least 2 events");
            assert!(model_events.lock().unwrap().len() >= 1,
                   "Model should have registered at least 1 event");
            assert!(api_events.lock().unwrap().len() >= 2,
