...
async fn test_basic_event_bus_functionality() -> Result<()> {
    ...
    event_bus.ui_sender().send(UiEvent::UserInput("ui test".to_string()))?;
    event_bus.model_sender().send(ModelEvent::ProcessUserMessage("model test".to_string()))?;
    ...
    assert!(model_processed, "Model event was not processed within timeout");
