...
use mcp_core::{debug_log, api_log};
...
impl CliApp {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        debug_log(&format!("Initializing Bedrock client with model: {}", self.config.model));
        ...
        let client = match BedrockClient::new(bedrock_config).await {
            ...
            Err(e) => {
                ...
                eprintln!("Also verify that the model ID '{}' is available in your AWS region.", self.config.model);
                            ...
                            match chunk_result {
                                Ok(chunk) => {
                                    ...
                                    if chunk.is_tool_call {
                                        debug_log(&format!("Tool call received: {:?}", chunk.tool_call));
