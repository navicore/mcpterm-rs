...
impl Colors {
    ...
    fn reset() -> &'static str {
        if Self::supports_color() { "\x1b[0m" } else { "" }
    }
    
    fn bold() -> &'static str {
        if Self::supports_color() { "\x1b[1m" } else { "" }
    }
    
    fn dim() -> &'static str {
        if Self::supports_color() { "\x1b[2m" } else { "" }
    }
    
    fn underline() -> &'static str {
        if Self::supports_color() { "\x1b[4m" } else { "" }
    }
    
    fn red() -> &'static str {
        if Self::supports_color() { "\x1b[31m" } else { "" }
    }
    
    fn green() -> &'static str {
        if Self::supports_color() { "\x1b[32m" } else { "" }
    }
    
    fn yellow() -> &'static str {
        if Self::supports_color() { "\x1b[33m" } else { "" }
    }
    
    fn blue() -> &'static str {
        if Self::supports_color() { "\x1b[34m" } else { "" }
    }
    
    fn magenta() -> &'static str {
        if Self::supports_color() { "\x1b[35m" } else { "" }
    }
    
    fn cyan() -> &'static str {
        if Self::supports_color() { "\x1b[36m" } else { "" }
    }
    
    fn white() -> &'static str {
        if Self::supports_color() { "\x1b[37m" } else { "" }
    }
}
...
impl ResponseFormatter {
    ...
    pub fn format_tool_result(result: &ToolResult) -> String {
        ...
        writeln!(&mut output, "{}┌─────────────────────────────────┐{}", Colors::cyan(), Colors::reset()).unwrap();
        writeln!(
            &mut output, 
            "{}│{} Tool Result: {}{}{}{} {}│{}", 
            Colors::cyan(),
        ...
        if let Some(err) = &result.error {
            writeln!(&mut output, "\n{}{}Error:{} {}", Colors::bold(), Colors::red(), Colors::reset(), err).unwrap();
        }
    ...
    fn format_shell_output(output: &mut String, result: &Value) -> () {
        ...
        writeln!(output, "{}{}Command:{} {}", Colors::bold(), Colors::blue(), Colors::reset(), command).unwrap();
        
        // Color exit code based on success/failure
        let exit_code_color = if exit_code == 0 { Colors::green() } else { Colors::red() };
        writeln!(
            output, 
            "{}{}Exit Code:{} {}{}{}", 
            Colors::bold(), 
        ...
        if !stdout.is_empty() {
            writeln!(output, "\n{}{}Output:{}", Colors::bold(), Colors::green(), Colors::reset()).unwrap();
            writeln!(output, "{}", &Self::format_command_output(stdout)).unwrap();
        }
        
        // Show stderr if present
        if !stderr.is_empty() {
            writeln!(output, "\n{}{}Errors:{}", Colors::bold(), Colors::red(), Colors::reset()).unwrap();
    ...
    fn format_file_output(output: &mut String, result: &Value) -> () {
        ...
        writeln!(output, "{}{}File:{} {}", Colors::bold(), Colors::blue(), Colors::reset(), path).unwrap();
        writeln!(output, "{}{}Size:{} {} bytes", Colors::bold(), Colors::blue(), Colors::reset(), size).unwrap();
        
        if !content.is_empty() {
            let preview_line_count = content.lines().take(20).count();
            let total_line_count = content.lines().count();
            
            writeln!(output, "\n{}{}Content:{}", Colors::bold(), Colors::green(), Colors::reset()).unwrap();
    ...
    fn format_write_output(output: &mut String, result: &Value) -> () {
        ...
        let status_color = if success { Colors::green() } else { Colors::red() };
        let status_text = if success { "Written successfully" } else { "Write failed" };
        writeln!(
            output, 
            "{}{}Status:{} {}{}{}", 
            Colors::bold(), 
        ...
        writeln!(output, "{}{}Bytes Written:{} {}", Colors::bold(), Colors::blue(), Colors::reset(), bytes_written).unwrap();
    }
    ...
    fn format_directory_output(output: &mut String, result: &Value) -> () {
        ...
        writeln!(output, "{}{}Directory:{} {}", Colors::bold(), Colors::blue(), Colors::reset(), path).unwrap();
        
        if let Some(entries) = entries {
            writeln!(output, "\n{}{}Contents:{}", Colors::bold(), Colors::green(), Colors::reset()).unwrap();
            ...
            writeln!(output, "\n{}No entries found{}", Colors::dim(), Colors::reset()).unwrap();
        }
    }
    ...
    fn format_generic_output(output: &mut String, result: &Value) -> () {
        ...
        match serde_json::to_string_pretty(result) {
            Ok(pretty_json) => {
                writeln!(output, "{}{}Result:{}", Colors::bold(), Colors::blue(), Colors::reset()).unwrap();
                // Add basic JSON syntax highlighting
                let highlighted = Self::highlight_json(&pretty_json);
                writeln!(output, "{}", highlighted).unwrap();
            }
            Err(_) => {
                writeln!(output, "{}{}Result:{} {}", Colors::bold(), Colors::blue(), Colors::reset(), result).unwrap();
            }
        }
    }
    ...
    fn highlight_json(json: &str) -> String {
        ...
        for c in json.chars() {
            match c {
                ...
                't' | 'f' | 'n' if !in_string && (json[json.find(c).unwrap()..].starts_with("true") || 
        ...
        match serde_json::from_str::<Value>(json_str) {
            Ok(json) => {
                if let Some(result) = json.get("result") {
                    ...
                    Some(format!("{}{}Error:{} {}", Colors::bold(), Colors::red(), Colors::reset(), error_message))
                } else {
            ...
            if let Some(result) = parsed.get("result") {
                ...
                if let Ok(result_pretty) = serde_json::to_string_pretty(result) {
                    ...
                    if result_pretty.contains("Here are") || result_pretty.contains("\\n") {
                        ...
                        let unescaped = result_str.replace("\\n", "\n")
                                                   .replace("\\\"", "\"")
                                                   .replace("\\\\", "\\");
                        return unescaped;
                    }
