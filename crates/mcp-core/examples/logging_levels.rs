use mcp_core::logging::tracing::{get_log_level, init_tracing};
...
fn main() {
    ...
    println!(
        "\nCheck the log file at {} to see the full logs.",
        log_file.display()
    );
    println!(
        "Note: Only messages at or above the current log level ({:?}) will be shown.",
        current_level
    );
}
