...
impl<T: ToolProvider> McpCommand<T> {
    ...
    /// Handle the 'schema' subcommand to show JSON-RPC schemas
    fn handle_schema(&self) -> CommandResult {
        let schemas = r#"
...
These schemas show the JSON-RPC format used by the Model Context Protocol.
They can be helpful for understanding the communication between your application and the LLM.
"#;
        CommandResult::success(schemas)
    }
}
