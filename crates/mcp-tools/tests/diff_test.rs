use mcp_tools::diff::{DiffConfig, DiffTool};
...
async fn test_diff_tool_whitespace_handling() {
    ...
    let result_sensitive = diff_tool
        .execute(json!({
            "old_content": old_content,
            "new_content": new_content,
    ...
    let result_insensitive = diff_tool
        .execute(json!({
            "old_content": old_content,
            "new_content": new_content,
    ...
    assert!(
        result_sensitive.output["stats"]["unchanged"]
            .as_i64()
            .unwrap()
            < 3
    );
    ...
    assert!(
        result_insensitive.output["stats"]["unchanged"]
            .as_i64()
            .unwrap()
            > result_sensitive.output["stats"]["unchanged"]
                .as_i64()
