use mcp_tools::diff::{PatchTool, PatchConfig};
...
";
    ...
    // Apply the patch
    let result = patch_tool.execute(json!({
...
async fn test_patch_tool_nonexistent_file() {
    ...
    assert_eq!(result.status, ToolStatus::Failure);
    assert!(result.output["error"].as_str().unwrap().contains("not exist"));
}