...
async fn test_find_tool_basic_search() -> Result<()> {
    ...
    println!("Find tool config: allowed_paths = {:?}", config.allowed_paths);
...
async fn test_find_tool_sorting() -> Result<()> {
    ...
    let large_file = create_test_file(&test_dir, "large.txt", "Large content that is even longer than the medium content")?;
...
async fn test_find_tool_denied_paths() -> Result<()> {
    ...
    assert!(!has_private, "Should not contain any file with 'private.txt'");
