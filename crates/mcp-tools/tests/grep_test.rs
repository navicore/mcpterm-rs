...
async fn test_grep_tool_simple_search() -> Result<()> {
    ...
    println!("GrepTool config: allowed_paths = {:?}", config.allowed_paths);
    ...
    // Verify the result
    assert_eq!(result.status, ToolStatus::Success, "Tool failed with error: {:?}", result.error);
    ...
    assert!(first_match["matched_text"].as_str().unwrap().contains("test"));
...
async fn test_grep_tool_with_include_pattern() -> Result<()> {
    ...
    create_test_file(&test_dir, "code.rs", "fn test_function() {\n    println!(\"This is a test\");\n}")?;
    create_test_file(&test_dir, "readme.md", "# Test Project\nThis is a test readme file.")?;
