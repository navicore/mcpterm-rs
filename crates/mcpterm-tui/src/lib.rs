...
use edtui::{EditorMode as EdtuiMode, EditorState, EditorTheme, EditorView, Lines};
use ratatui::{
...
impl AppState {
    fn new() -> Self {
        ...
        let messages = vec![
            ...
            Message::new("Tab: Switch focus".to_string(), MessageType::System),
            Message::new("i: Enter insert mode".to_string(), MessageType::System),
            Message::new(
            ...
            Message::new("a: Toggle auto-scroll".to_string(), MessageType::System),
            Message::new(
            ...
            Message::new("q: Quit (in normal mode)".to_string(), MessageType::System),
    ...
    fn scroll_to_bottom(&mut self) {
        ...
        let max_scroll = self
            .messages
            .len()
            .saturating_sub(self.visible_message_count);
    ...
    fn navigate_history(&mut self, direction: KeyCode) {
        ...
        match direction {
            ...
            KeyCode::Down if self.history_index < self.history.len() - 1 => {
                self.history_index += 1;
                self.input = self.history[self.history_index].clone();
            }
            KeyCode::Down if self.history_index == self.history.len() - 1 => {
...
fn ui(f: &mut ratatui::Frame, state: &mut AppState) {
    ...
    let messages_block = Block::default()
        .title(format!(
            "Messages ({}/{}) - {}",
            state.scroll, max_scroll, auto_scroll_indicator
        ))
        .borders(Borders::ALL)
    ...
    let input_block = Block::default()
        .title(format!(
            "Input ({}) | Last key: {}",
            mode_str, state.last_key
        ))
        .borders(Borders::ALL)
        .border_style(input_border_style);

    // Create the input editor widget using edtui
    let input_editor = InputEditor::new(&state.input, state.mode).block(input_block);
