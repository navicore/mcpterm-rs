...
// The UI is now rendered directly in the main function
...
fn main() -> Result<()> {
    ...
    // Main loop
    while state.running {
        // Render the UI with a temporary variable to avoid multiple mutable borrows
        {
            let mut message_viewer = MessageViewer::new(&mut state.message_view_state);
            ...
            terminal.draw(|f| {
                // Create local references to state that will be captured by the closure
                let running = &state.running;
                let focus = &state.focus;
                let editor_mode = &state.editor_mode;
                let messages_scroll = &state.messages_scroll;
                ...
                let message_border_style = if *focus == Focus::Messages {
                ...
                let messages_block = Block::default()
                    .title(format!("Messages (scroll: {})", messages_scroll))
                    .borders(Borders::ALL)
                    .border_style(message_border_style);
                
                // Set block for message viewer
                message_viewer.block = Some(messages_block);
                
                // Render message viewer directly
                f.render_widget(MessageViewer {
                    state: message_viewer.state,
                    block: message_viewer.block.clone(),
                }, chunks[0]);
                ...
                let input_block = Block::default()
                    .title(format!("Input ({}) | Last key: {}", mode_str, last_key))
                    .borders(Borders::ALL)
                    .border_style(input_border_style);
                
                // Set block for input editor
                input_editor.block = Some(input_block);
                
                // Set correct mode for input editor
                input_editor.set_mode(*editor_mode);
                
                // Render input editor directly
                f.render_widget(InputEditor {
                    state: input_editor.state,
                    block: input_editor.block.clone(),
                    event_handler: input_editor.event_handler,
                }, chunks[1]);
            })?;
        }
