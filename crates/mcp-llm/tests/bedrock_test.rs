...
async fn test_bedrock_client_with_mcp_validation() -> Result<()> {
    ...
    assert!(
        ...
        "Response should be valid MCP format: {:?}",
        validation_result
    );

    // JSON-RPC specific validation
    assert!(
        json_value.get("jsonrpc").is_some(),
        "Should have jsonrpc field"
    );
    assert_eq!(
        json_value["jsonrpc"].as_str().unwrap(),
        "2.0",
        "Should have jsonrpc version 2.0"
    );
...
async fn test_bedrock_streaming_with_mcp() -> Result<()> {
    ...
    if !complete_content.is_empty() {
        ...
        if let Ok(json_value) = json_result {
            ...
            assert!(
                validation_result.is_ok(),
                "Final streamed content should be valid MCP format: {:?}",
                validation_result
            );
            ...
            println!(
                "Warning: Final streamed content is not valid JSON: {}",
                complete_content
            );
...
async fn test_bedrock_tool_call_with_mcp() -> Result<()> {
    ...
    if !response.tool_calls.is_empty() {
        ...
        assert!(
            tool_call.params.get("command").is_some(),
            "Should have command parameter"
        );
        ...
        if let Ok(json_value) = json_result {
            ...
            if json_value.get("method").is_some() {
                assert_eq!(
                    json_value["method"].as_str().unwrap(),
                    "mcp.tool_call",
                ...
                assert!(
                    params.get("parameters").is_some(),
                    "Should have tool parameters"
                );
            }
        }
    }
